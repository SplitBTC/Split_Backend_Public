...
async function resolveAuthenticatedWalletPubkey(req) {
  const tokenPubkey = normalizePrivacyWalletPubkey(req.pubkey);
  if (tokenPubkey) {
    return tokenPubkey;
  }

  const user = await User.findById(req.userId).select('_id walletPubkey');
  if (!user) {
    return null;
  }

  return normalizePrivacyWalletPubkey(user.walletPubkey);
}
...
async function loadActiveMessagingBindingV4(account) {
  ...
  return MessagingBinding.findOne({
    messagingAccountId: account._id,
    active: true,
  }).sort({ updatedAt: -1, createdAt: -1 });
}

async function loadAuthenticatedMessagingIdentityV4(req) {
  const walletPubkey = await resolveAuthenticatedWalletPubkey(req);
  if (!walletPubkey) {
    return {
      error: {
        status: 401,
...
async function handleMessagingIdentityV4Get(req, res) {
  try {
    ...
    if (/MESSAGING_DATA_PEPPER is required/.test(String(error?.message || error))) {
...
async function handleMessagingIdentityV4Post(req, res) {
  try {
    ...
    const binding = normalized.binding;
    const authenticatedWalletPubkey = await resolveAuthenticatedWalletPubkey(req);
    if (!authenticatedWalletPubkey) {
      return res.status(401).json({ error: 'Unauthorized' });
    }

    if (authenticatedWalletPubkey !== binding.walletPubkey) {
      return res.status(403).json({ error: 'walletPubkey does not match the authenticated user' });
    }
    ...
    if (didUpdate) {
      ...
      storedBinding = await MessagingBinding.create({
        ...
        lightningAddressHashScheme: binding.lightningAddressHashScheme,
        messagingPubkey: binding.messagingPubkey,
        messagingPubkeyMessagingHmac: buildMessagingPubkeyHmac(binding.messagingPubkey),
        messagingIdentitySignature: binding.messagingIdentitySignature,
        messagingIdentitySignatureVersion: binding.messagingIdentitySignatureVersion,
        messagingIdentitySignedAt: binding.messagingIdentitySignedAtDate,
        active: true,
...
async function handleMessagingDeviceRegistrationV4Post(req, res) {
  try {
    ...
    if (/MESSAGING_DATA_PEPPER|PUSH_TOKEN_LOOKUP_PEPPER|PUSH_TOKEN_ENCRYPTION_KEY/.test(message)) {
      return res.status(500).json({ error: 'Messaging privacy configuration is missing' });
    }
